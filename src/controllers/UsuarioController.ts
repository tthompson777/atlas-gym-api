...
export class UsuarioController {
  ...
  async excluirUsuario(req: Request, res: Response, next: NextFunction) {
    if (!req.user || req.user.role !== 0) {
    ...
    try {
      ...
      // Opcional: Impedir que o usuário se delete (embora o frontend possa tratar isso)
      if (req.user && usuario.id === req.user.userId) {
      ...
      if (usuario.role === 0 && adminCount <= 1) {
        res.status(400).json({ error: 'Não é possível excluir o único administrador da empresa.' });
        return;
      }
